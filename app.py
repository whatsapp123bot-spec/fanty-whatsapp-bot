# Compatibility WSGI entry for Render services still configured with `gunicorn app:app`.import os

# This is NOT Flask. It simply exposes the Django WSGI application as `app`.import io

from mi_chatfuel.wsgi import application as appimport requests
...
def ensure_flow_node_column():
    """Garantiza que la columna flow_node exista en users (migración perezosa)."""
    try:
                            ...
                            if msg_type == 'text':
                                ...
                                try:
                                    if 'cerrar flujo' in low:
                                        ...
                                        # No auto-iniciar; esperar a que el usuario use un disparador (keywords/deeplink/IA)
                                        send_whatsapp_text(from_wa, '✅ Flujo cerrado. Escribe una palabra clave o link para iniciar de nuevo.')
                                        return 'ok', 200
                                except Exception:
                                    pass
